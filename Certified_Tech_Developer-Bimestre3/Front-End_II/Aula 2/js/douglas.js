...
for (var rodada; rodada <= 3; rodada++) {
    start();
}
...
function start() {
    if (confirmar == false) {
        ...
        if (numUsuario == 'PEDRA') {
            ...
            alert(
                'Como você não digitou um valor valido, vou jogar por você ;)'
            );
    ...
    if (numUsuario == numComputador) {
        ...
        numUsuario - numComputador == 1 ||
        numUsuario - numComputador == -2
    ) {
        this.vitoriaUsuario = vitoriaUsuario + 1;
        alert('O Jogador 1 Venceu!');
    } else if (
        numUsuario - numComputador == -1 ||
        numUsuario - numComputador == 2
    ) {
        this.vitoriaComputador = vitoriaComputador + 1;
